...
class TestProjectIngestion(unittest.TestCase):
    ...
    def test_validate_git_url(self):
        ...
        self.assertFalse(
            validate_git_url("http://gitlab.com/user/repo.git")
        )  # HTTP rejected
        self.assertFalse(
            validate_git_url("file:///etc/passwd")
        )  # file:// rejected
        self.assertFalse(
            validate_git_url("ssh://git@github.com/user/repo")
        )  # ssh:// rejected
        self.assertFalse(
            validate_git_url("/local/path/to/repo")
        )  # local filesystem path rejected
        self.assertFalse(
            validate_git_url("git://github.com/user/repo")
        )  # git:// rejected
        self.assertFalse(
            validate_git_url("git@github.com:user/repo.git")
        )  # git@ transport rejected
        self.assertFalse(
            validate_git_url("-oProxyCommand=abc")
        )  # Unsafe arguments
    ...
    def test_validate_website_url(self):
        ...
        self.assertTrue(
            validate_website_url("http://127.0.0.1:8080/test")
        )
        self.assertFalse(
            validate_website_url("file:///usr/bin/local")
        )
        self.assertFalse(
    ...
    def test_website_registration(self):
        ...
        project = handle_website_ingestion(
            self.project.id,
            "https://example.com",
            self.db,
        )
    ...
    def test_safe_zip_extraction(self):
        ...
        self.assertTrue(
            (target_dir / "src" / "main.py").exists()
        )
        self.assertTrue(
            (target_dir / "README.md").exists()
        )
    ...
    def test_router_ingest_website(self):
        ...
        payload = WebsiteIngestRequest(
            target_url="https://kyptic.io"
        )

        with patch(
            "app.services.ingestion_service.is_ssrf_safe_url",
            return_value=(
                True,
                "URL is SSRF safe.",
            ),
        ):
            project = ingest_website(
                self.project.id,
                payload,
                self.db,
            )
